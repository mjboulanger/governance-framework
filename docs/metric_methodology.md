...
**Full-attribution requirement [LOCKED 2026-08-10 - binds the D6/D7 build].** Every score the framework emits MUST be fully decomposable to source: no value that feeds a score is discarded, and every intermediate (multiplier, fraction, floor, weight) is PERSISTED alongside the score it produced, using the actual computed values. The dashboard requirement this serves: a user clicks into any concept score and sees exactly how it was built - each metric's raw + normalized + harmonized value, latest year + stale flag, tier + tier-weight, present/missing + missingness regime, and contribution to the tier-weighted mean; then the concept-level chain - computed (pre-penalty) score, the missingness penalty (f = penalty-relevant fraction present, floor, w, regime, penalized score), the relevance multiplier, the measurement-quality multiplier, the effective weight (relevance x MQ), and the concept's weighted contribution to its category; then the category roll-up (each concept's weighted contribution, summed to the category score). Three persisted attribution tables carry this: (1) **metric-level** (extends `concept_contributions.csv` - add the missingness tag/regime per metric); (2) **concept-level** (NEW: one row per iso3 x concept with computed_score, f, floor, w, penalty_regime, penalized_score, relevance, measurement_quality, effective_weight, weighted_contribution, low_confidence); (3) **category-level** (NEW: one row per iso3 x category with each concept's weighted contribution and the summed category score). Intermediates are stored EVEN WHEN a step is a no-op (e.g. f>=0.5 -> record f and 'penalty: none', not a blank), so 'why is there no penalty here' is answerable and no populated-vs-empty ambiguity arises. Nothing is recomputed on demand or hidden.
