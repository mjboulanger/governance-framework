...
**Common-scale harmonization [LOCKED - 2026-07-22].** The method families above output on incompatible scales (z-family approx -3..+3 centered 0; percentile 0-1; occurrence 0/1; fixed-anchor 0-1), so a plain tier-weighted mean across families (S6) would be dominated by the z-family spread and offset by its differing center. All normalized metrics are therefore mapped to a **common 0-1 scale** before aggregation: z-family values are **linearly rescaled** as (clip(z,-3,+3)+3)/6; percentile, occurrence and fixed-anchor values pass through unchanged. The clip reuses the existing +/-3 winsorization bound. The transform is **linear in z**, preserving the distance information S5 deliberately protects (ceiling-piled development indicators kept on z rather than percentile); converting z to percentile at this step would undo that choice and is rejected.

**Residual spread asymmetry [ACCEPTED, flagged for Step 4].** Harmonizing range does not equalize variance: post-transform SDs are approx 0.167 (z-family), 0.289 (percentile), 0.5 (binary). A tier-1 binary therefore swings a concept score roughly 3x as hard as a z-scored metric. This is accepted rather than corrected - forcing equal SD would require z-scoring everything at the aggregation step, destroying the absolute property that fixed-anchor and binary metrics exist to provide. **Step-4 check:** verify no concept score is driven predominantly by a single binary or fixed-anchor metric.

**Winsorization bound (+/-3) is a DEFAULT [revisit Step 4].** Widening to +/-4 is only coherent if the S5 winsorization moves too; rescaling +/-3-winsorized values against a +/-4 bound would strand 25% of the scale and compress all values toward 0.5. Whether +/-4 is warranted is an empirical question - how much real mass sits beyond 3 SD per metric - checkable against metric_distribution_profile.csv at Step 4.
