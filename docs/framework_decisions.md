...
### ASCOR composite specification (2026-07-22)
Decided in working session after inspecting the delivered data (TPI ASCOR export, 21/07/2026; latest assessment 18/08/2025, 85 countries). ASCOR publishes **no composite score** — the hierarchy tops out at 14 areas (Yes/Partial/No), with no pillar-level or overall roll-up — so the framework constructs its own.

**Spec: ONE metric, built from 9 equally-weighted areas.**
- Areas: **EP.1, CP.1, CP.2, CP.3, CP.4, CP.5, CP.6, CF.2, CF.3**
- Each area = share of its **applicable** (Yes/No) indicators answered Yes
- **Deduplicate indicator codes first** — the results file contains `CP.2.c` ×4 and `CF.1.b` ×3 (version drift across the 2023/2024/2025 rounds); a naive share-of-Yes would quadruple-count one carbon-pricing question
- Exempt / Not applicable / No data are **excluded from denominators** (not scored as No)
- Areas with no applicable indicators for a country **drop out; renormalize over remaining areas** (consistent with D5)
- Result: ~22–24 applicable indicators per country, fixed **0–1 anchor**, entered under the §5 **fixed-anchor** family (pass through unnormalized)

**Why area-level roll-up rather than raw share-of-Yes across all indicators:** indicator count per area reflects how decomposable ASCOR found the topic, not its importance (CP.2 has 6 indicator slots vs CP.1's 2). Equal-weighting areas prevents that arbitrary weighting, and is robust to the duplicate-code problem.

**Why one metric, not three (one per pillar):** (a) CF is too thin to stand alone — median 4 applicable indicators, 16 countries have ≤2, so a CF composite takes values like {0, 0.5, 1}; (b) three ASCOR metrics would make ASCOR ~40–45% of C12 under equal-weight-within-tier, disproportionate for a source absent from 56% of sovereigns; (c) the all-48 composite correlates 0.872 with CP alone, so little is lost by concentrating on the governance content.

**Exclusions.** **EP.2/EP.3/EP.4 (targets)** — discriminating variance comes from *ambition benchmarking* ("is the target aligned with the country's 1.5°C fair share?"), a climate-equity judgment against a scientific standard rather than governance quality; the genuinely governance part (has a target been set) is near-universal (EP.2.a 93% yes) or better captured by CP.1, which asks whether a framework climate law specifies accountability elements. Five EP indicators sit at 2–6% yes — near-degenerate. **CF.1** — structurally a donor question ("does the country contribute its proportional share of the $100bn commitment?"), 37% exempt; scoring EMs on it measures whether they are rich enough to be donors. **CF.4** — contains no Yes/No indicators (metrics only). All retained in the evidentiary layer.

**Why EP.1 IS included, despite being an outcome.** All eight other areas are **de jure** — has a law, published a plan, made a commitment, disclosed a breakdown. A country could score 1.0 with a complete policy suite and no delivery: the BRSS rules-on-paper failure mode. EP.1 ("has the country improved its emissions profile over the past 5 years?") is the only **de facto** leg ASCOR offers, and per the construct-validity principle outcomes often proxy governance better than process. EP correlates only 0.328 with CP, so it adds distinct information. **Caveats:** EP.1 is not uniformly strong — EP.1.a (55% yes) is the real outcome check, while EP.1.b (6%) and EP.1.c (39%) are alignment-benchmark judgments; and a 5-year emissions trend is confounded by growth rate, industrial structure, offshoring and the COVID collapse/rebound. **Flagged for the wealth/structure audit.** At 1 of 9 areas (~11%), the composite remains overwhelmingly de jure — state plainly in any output documentation.

**Tier: leans P2, confirm at Step 1.** 85 countries = 44% of the 192-sovereign core, below the 60% inclusion threshold (flags for review, not auto-drop). Composition: 50 high-income, 21 upper-middle, 13 lower-middle, 1 low-income — only **35 non-high-income**; regionally Sub-Saharan Africa 6, South Asia 4. Strong EM-index depth, thin frontier/LIC coverage.

**Reference-class distortion — why fixed-anchor matters here.** ASCOR's sample is advanced-skewed (mean share-of-Yes 0.553 advanced vs 0.357 emerging, an 18-point gap). Under distributional normalization a typical EM would score z ≈ −0.48 against the 85, roughly 0.05 lower on a 0–1 scale than against a representative reference class — penalised for *being measured alongside rich countries*, making inclusion in ASCOR a disadvantage relative to omission. Fixed-anchor pass-through eliminates this: the score depends only on the country's own answers.

**Panel/momentum.** Three rounds: 2023 (25 countries), 2024 (70), 2025 (85). Momentum requires ≥3 panel points, so it is computable for only **25 of 85** countries and null for the rest.

**Data-cleaning requirements.** Normalize casing before parsing — the raw export contains both `No Data`/`No data` and `Not applicable`/`Not Applicable`. Genuinely missing data is low (~1.7%); Exempt is 14% and is a distinct state, not missingness.
