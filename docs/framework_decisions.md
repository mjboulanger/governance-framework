...
| **9 · Financial-sector regulatory & supervisory quality** | **✅ Built:** —<br>**🟡 Outstanding:** IMF/WB FSAP `[classified PDF-batch — but 'PDF-only' assumed, never probed]` · FATF Mutual Evaluations `[classified scrape, unprobed]` · Basel AML Index `[parked: requires institutional affiliation]` · Basel Core Principles (BCP) `[unexamined — no access investigation]` · IOSCO Principles `[unexamined — no access investigation]` · IAIS Insurance Core Principles `[unexamined — no access investigation]`<br>**⚪ Closed:** — |
...
| **19 · Legislative & constitutional checks** | **✅ Built:** V-Dem v2xlg_legcon + components *(vdem)* · CCP separation-of-powers *(QoG ccp_government_system)* · Polity5 *(QoG — composite, not XCONST sub-component specifically)*<br>**🟡 Outstanding:** IPU Parline `[classified scrape, unprobed]`<br>**⚪ Closed:** — |
...
| **23 · Media freedom & pluralism** | **✅ Built:** V-Dem media v2x_freexp_altinf… *(vdem)* · FH-FIW CL-D *(fh)* · RTI Rating *(rti_rating)*<br>**🟡 Outstanding:** CPJ journalist-safety `[classified scrape, unprobed]`<br>**⚪ Closed:** RSF World Press Freedom Index *(dropped — V-Dem covers; methodology break)* |
...
**Headline:** ~10 concepts fully built; most of the rest are tier-1-complete with gaps only in 3rd/supplementary legs. **Concept 9 (financial-sector regulatory quality) is the one substantially-unbuilt concept** — all 6 primaries are PDF/scrape/blocked/unexamined. Genuine tier-1 build gaps are narrow: Concept 9 (all), ACLED (C2), AREAER de-facto (C8), WTO TFA (C11), IPU Parline (C19), IDEA EMB (C20), CPJ (C23). The `classified, unprobed` items (FSAP, FATF, IMF SDDS, IPU Parline, CPJ) are the highest-value scout targets — prior "hard" verdicts on TFI/NTM proved wrong on inspection.
