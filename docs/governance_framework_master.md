...
| | Macroeconomic policy framework quality | Romelli CBI Index; IMF Fiscal Rules Database; IMF AREAER (FARI); Chinn-Ito KAOPEN; IMF iMaPP; Reinhart-Rogoff exchange rate | Dincer-Eichengreen CB Transparency (deprioritized); Heritage Monetary Freedom / Fraser Sound Money (monetary only); EIU Country Risk macro (optional paid) |
...
| Source | Decision | Rationale / notes |
|--------|----------|-------------------|
| PEFA (Public Expenditure and Financial Accountability) | Primary tier 1 | Gold standard PFM assessment across 31 indicators. ~150 countries with at least one assessment; ~75 with multiples. Tier 3 extraction (PDF reports + Secretariat portal). |
...
| Chinn-Ito KAOPEN | Primary tier 1 (derivative) | Most-cited academic capital-account-openness index, derived from AREAER. Automated, ~181 countries 1970–2023. Broad time-series complement / cross-check to FARI; not more authoritative than AREAER. |
| IMF iMaPP (Integrated Macroprudential Policy Database) | Primary tier 1 | Macroprudential measure adoption. ~130 countries. |
| Reinhart-Rogoff exchange rate classifications | Primary tier 1 | De facto exchange rate regime. Complements AREAER's de jure focus. ~190 countries. |
...
**State of measurement:** strong. Multiple direct institutional measures across all sub-dimensions (central bank, fiscal framework, exchange regime, macroprudential). Capital-account/exchange-rate governance is now triangulated across IMF-native (AREAER FARI), academic derivative (Chinn-Ito), and de facto regime (Reinhart-Rogoff).
