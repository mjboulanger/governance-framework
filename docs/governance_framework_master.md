...
- **Concept 25 (Government transparency and openness) reconsideration.** Significant indicator overlap with other concepts (only the IDEA Political Finance Database and Global Data Barometer are unique to it). **Decision (revisited 2026-06-18): KEEP as own concept.** "Government transparency and openness" is a coherent, investor-legible dimension; overlap tracked under the repetition rule. The Global Data Barometer is deprioritized as its open-data source (thin, edition-unstable, duplicates ODIN). **Finalized 2026-08-11:** the full-framework revisit was performed after scoring + validation. Empirical redundancy test - C25 regressed on its 4 source-overlapping concepts (C8 Macroeconomic and financial policy framework, C14 Legal quality and predictability, C21 Political participation beyond voting, C23 Media freedom and pluralism) gives R2 = 0.617, so 38%% of C25's variance is independent - below the redundancy bar. KEEP STANDALONE confirmed. Honest limitation: only 12%% of C25's score comes from its 2 unique metrics (polfin_transparency_integrity, odin_overall); its independence rests on a distinct combination of largely-shared sources plus conceptual coherence, not unique measurement. See `framework_decisions.md` for the full test.
...
| ACLED | **Deferred — not scored (v1)** | Real-time event data; broader event types than UCDP (protests, riots, civilian violence). Access is research-tier API, approval pending (see build-status). NOT in v1 scoring; C2 stability is scored without it. Re-entry if API access clears. |
...
**State of measurement:** strong. Multiple methodologically distinct sources cover stability outcomes well. Triangulation across WGI aggregate, V-Dem expert coding, and event-based data (UCDP, Powell-Thyne coups) is robust (ACLED deferred, not scored in v1). Note on scored form: the Powell-Thyne coup metrics are scored as **10-year trailing-window counts** (pt_coup_successful + pt_coup_failed, not raw single-year events), and UCDP is scored as **log1p(intrastate battle deaths per 100k population)** (ucdp_sb_intrastate_deaths_best, not raw death counts) - derivations built in `src/derive_metrics.py`, spec in `metric_methodology.md`.
...
| Source | Decision | Rationale / notes |
|--------|----------|-------------------|
| Reporters Without Borders (RSF) — World Press Freedom Index | **Deprioritized — not scored (v1)** | Standard cross-country press freedom measure (180 countries, annual). NOT scored in v1: media-freedom climate is covered by V-Dem's press-freedom indicators (5 scored metrics) plus CPJ event-counts and RTI; RSF adds a methodology-break (2021 scale revision) without distinct coverage. C23 is scored without RSF. |
...
**Note on concept status:** significant indicator overlap with other concepts (only the IDEA Political Finance Database and Global Data Barometer are unique to this concept; the rest are also used elsewhere). **Decision: KEEP as its own concept** (finalized 2026-08-11) — coherent, investor-legible dimension; overlap tracked under the repetition rule. The full-framework revisit was performed after scoring + validation: C25 regressed on its 4 source-overlapping concepts (C8, C14, C21, C23) gives R2 = 0.617, so 38%% of its variance is independent - below the redundancy bar, keep-standalone empirically confirmed. Honest limitation: only 12%% of the score comes from C25's 2 unique metrics (polfin_transparency_integrity, odin_overall); independence rests on a distinct combination of largely-shared sources plus conceptual coherence.
...
- **Concept 25 (Government transparency and openness) — reconsideration FINALIZED, keep standalone.** The full-framework revisit (gated to finalisation) was run after scoring + validation: C25 regressed on its 4 source-overlapping concepts (C8, C14, C21, C23) gives R2 = 0.617 (38%% independent variance, below the redundancy bar). Keep-standalone empirically confirmed; honest limitation recorded (only 12%% of score from the 2 unique metrics). See `framework_decisions.md`.
