...
| | Trade governance | WB Logistics Performance Index; OECD Trade Facilitation Indicators; WTO TFA implementation; KOF Globalisation Index Trade subindex; WB WITS tariff data; Heritage Trade Freedom | WTO Trade Policy Reviews (Tier 3); WTO RTA database |
...
| WTO Trade Facilitation Agreement (TFA) implementation tracking | Primary tier 1 | **Not built** — planned Category-3 web scrape (tfadatabase.org), not yet attempted. Country commitments and implementation status. All WTO members. Continuously updated. The one genuinely outstanding primary in this concept. |
...
**State of measurement (build-verified):** mostly built. **Administration:** LPI ✅ (`wdi_clean.csv`) + TFI ✅ (`tfi_clean.csv`) built; WTO TFA ❌ not built (planned Cat-3 scrape) — so 2 of 3 admin primaries built, already triangulable, with TFA the one outstanding admin source. **Openness:** tariffs ✅ (`wdi_clean.csv`) + Fraser Trade ✅ (`fraser_clean.csv`) built; KOF proxied via QoG `dr_eg` (⚠️ mismatch, metric-pass decision); **non-tariff barriers are an accepted v1 gap** — UNCTAD NTM dropped on currency (2012-2017, ~76 countries). TFI built at **composite-average** level (sufficient — admin triangulation is real, not just designed); A–K breakdown a possible enhancement, not a v1 requirement. Net: only WTO TFA (build) and the KOF proxy-vs-dedicated call (metric pass) remain.
