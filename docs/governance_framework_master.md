...
**Directionality [OPEN — D3]:** more state control is expected to score as worse governance, but **likely non-monotonic** — a healthy baseline of state economic role (public goods, natural monopolies, strategic sectors) is not governance-negative; the penalty should activate only above a threshold. Exact transform (monotonic vs. threshold/band) deferred to the D3 sign-pass, set against the observed v2clstown distribution rather than assumed blind.
