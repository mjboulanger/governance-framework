...
(tracked). Peers are selected by similarity on:

- **GDP per capita (USD nominal)** - the primary axis (log distance).
- **Region** - a soft lean toward same/near region (not a hard filter, so income-similar
  countries from other regions still appear - e.g. Australia is a US peer).
- **Population** - a soft lean toward similar-size countries (keeps large economies from
  being matched only to tiny income-twins).
